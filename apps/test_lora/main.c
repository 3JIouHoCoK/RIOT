...
static int _dac_set_cmd(int argc, char **argv){
     if (argc != 1) {
        printf("Usage dac_set <value>");
        return 1;
    }
    uint16_t dac_value = atoi(argv[1]);
    if (dac_value < 50000){
        printf("Incorrect DAC value. Set 50000-65535\n");
        return 1;
    }
    dac_set(DAC_LINE(0), dac_value);
    return 0;
}
...
static const shell_command_t shell_commands[] = {
    ...
    { "dac_set", "Set DAC value ~50000-65535", _dac_set_cmd},
    { NULL, NULL, NULL }
};
